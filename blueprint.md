...
> 修订日期：2026-09-15
>
> 文档性质：产品、领域、模型与实施规划。B0 已按 1.13 落地；1.14 写入图谱产品形态；1.15 把全书 TXT 结构与阅读器提前，PDF/OCR 延后到阶段 P。
>
> 核心路线：文档 → 事件式语义单元（Event-like EDU）→ 实体关联图（Entity Association Graph）→ 检索与生成 → 原文引用
>
> 图谱产品讨论底稿：[graph-grok.md](./graph-grok.md)。全书结构与 Text Unit：[textunit.md](./textunit.md)。与本文冲突时以本文为准。
...
| 产品与工程 | [工作台](#16-产品界面与工作流)、[图谱视图](#169-图谱视图)、[架构](#17-系统架构)、[技术栈](#172-技术栈选型)、[数据](#18-概念数据结构)、[接口](#19-api-与处理任务) |
...
本规划定义产品、领域与实施边界。仓库已按 1.13 落地 B0。1.15 不重开 B0：后续第一包是全书 TXT 的结构方案、目录树和三栏阅读器；PDF、MinerU 与 OCR 放到阶段 P，排在写作之后。

B0 已实现内容以代码和迁移为准，不再把「仓库没有应用代码」当作前提。后续阶段仍以本文为上位方案；`dev-landing.md` 负责如何写成代码。不能为了兼容讨论稿或外部产品而恢复已明确删除的复杂层次。
...
- 显式「合并图」作业或第二份物化跨文档图；跨文档连接靠共享 Entity，三种 scope 只换有权文档版本集合；
- 为实体自动生成综述词条或人物传记；实体页、事件页和检查器是叙事出口；
- 默认按自动语义社区给图画色或分组裁决；首期着色只有类型和来源文档两种模式。
...
| D08 | 实体图从文档级 EDU 和文档级实体链接投影           | 图查询只有一种实现；文档/项目/全局三种 scope 只换有权文档版本集合，不复制链接、不另建合并图 |
...
| D21 | 图谱是 EDU 脱离原文顺序后的浏览界面              | 不新增第六个一级入口；画布放在「实体与事件」下；阅读器仍按原文顺序消费 EDU |
| D22 | 未链接事件要素作为显示节点进入画布                | 不建第三类领域对象或通用节点表；冷启动时图不可因宁缺毋错而空白            |
| D23 | 首期画布只画 EDU→实体（含未链接）二部图           | 不画 EDU–EDU 边和实体间快捷边；关系型引用用列表和事件组呈现             |
| D24 | 共享实体表预留工作空间边界列                     | 单租户用常量占位并纳入唯一约束；项目不拥有实体；不做跨租户实体池           |
...
用户导入文件、网页快照或纯文本，补充题名、作者、出版信息和材料类型。平台先保存原始资产和文档记录。史书等全书 txt 在提取前选择结构方案并预览目录；确认后才生成不可变文档版本和文本单元。无结构方案的短文本仍可按空行切分后提取。

重复内容只提示，不阻止上传。相同内容可以复用底层资产，但不同用户、书目信息和权限仍保留各自的文档记录。

文档详情提供两个显式操作：

- **提取文档内容**：生成新的不可变文档版本、文本单元和页图映射。TXT、Markdown 使用平台内置文本导入器与结构方案，不调用外部解析器；PDF、Office、图片和 HTML 在阶段 P 调用选定解析器。
- **抽取 EDU**：对选定文档版本和范围创建批量任务，按文本单元与分层上下文生成、校验并复核 EDU。范围可以是 path 前缀（本卷、本篇、本年）。全书默认不得一键全文抽取，须选到更细范围或明确确认预算。
...
阅读器按原文顺序消费 EDU。图谱是 EDU 脱离原文顺序后的第二界面：检索让 EDU 可查，图谱让 EDU 可逛。
...
也可以在图谱画布上沿 `EDU → 实体` 边走动，从实体页、事件页和画布检查器进入同一份 EDU，再回到原文。

事件浏览以事件型 EDU 为基本记录，不要求先把多个来源强行合并为一个“真实事件”。跨来源材料用查询时聚合的事件组呈现，不新建事件簇对象。
...
- 有文档查看权限但无编辑权限时，对固定文档版本中的实体链接提交轻量纠错建议；
...
| 实体（Entity）              | 在平台内跨文档、项目和 EDU 复用的身份索引记录         | 不直接保存有争议的事件事实，不归属于单一项目 |
...
| 事件簇（Event Cluster）    | 查询时聚合为事件组，不落表     |
| 通用图节点 / 图边          | 由 EDU 参数和文档级链接投影   |
...
首版按单一工作空间部署，不引入组织层级或工作空间切换界面。Entity 不属于任何项目，而属于当前工作空间的共享实体池。阶段 C 起，`entity`、`entity_name` 等表带工作空间边界列（`tenant_id`），单租户用常量占位并纳入唯一约束。项目关注实体只是项目对一批 Entity 的可选标记，不建立另一份 Entity 副本。添加或移除关注只改变人工候选展示、查询扩展和结果排序，不修改 Entity、既有 EDU 或实体链接。

图查询的 scope 是「当前用户有权访问的文档版本集合」，不是另一份图数据。文档图、项目图、全局图只换这个集合：一份固定版本、项目所选版本、或用户有权的全部版本。共享 Entity 是跨文档 join 键，不另做合并图作业。
...
Entity 作为身份索引记录在平台内共享，实体链接结论绑定固定文档版本中的 EDU 参数：文档级 `edu_argument` 保留角色、原文写法、规范值和建议类型，文档级链接记录把具体参数连接到共享 Entity。同一文档版本被多个项目使用时复用相同链接，项目不生成或覆盖自己的链接。候选生成和排序可以使用共享实体池中的名称、别名、类型及既有关联线索；最终自动链接必须由该固定文档版本的内容提供足以区分候选的依据。结论依赖其他文档中的争议性陈述或项目研究立场时保持未链接，相应身份假说保存在研究成果中。
...
卷、纪、年、回是篇章节点，写入 `path` 前缀并聚合成目录树，不是默认的唯一文本单元。编年体按年导航，按段落 unit。结构规则由一书一份结构方案给出，解析预览确认后再写版本。
...
以上共享实体池信息用于产生和排序候选；自动链接是否成立，仍以固定文档版本中的名称、指代、类型和上下文能否可靠区分候选为准。其他文档中的争议性陈述或项目研究立场不能作为强制形成文档级链接的依据。
...
有文档查看权限但无编辑权限的用户可以提交文档级实体链接纠错建议，指向被质疑的 EDU 参数和当前链接修订，保存原因及可选的建议 Entity。纠错建议不修改现有链接、图投影或检索结果，由文档编辑者采纳或驳回；采纳时仍按普通改链规则校验修订号并记录审计信息。纠错建议与系统产生的未链接候选分别保存和统计，不建设通用审批系统、任务分派或通知中心。
...
合并界面必须说明该操作修改的是平台共享身份索引，会影响所有相关文档版本后续的实体检索和图投影；规范显示名和别称不应被表现为人物传记或平台对争议身份的权威裁定。
...
若对照外部代码图谱产品理解范围，概念对应如下。这只帮助划界，不把对方的对象搬进本平台：

| 外部概念 | 本平台对应 |
| --- | --- |
| 工作空间 | 共享实体池边界（表列 `tenant_id` 占位）；首期不提供工作空间切换界面 |
| 被索引的仓库 | 固定文档版本，是抽取单元，不是研究项目 |
| 跨仓合并图 | 项目图或全局图；同一套查询换有权文档版本集合，不另跑合并作业 |
| 由代码图生成的 Wiki | 实体页与事件页；学图与页互跳，不生成人物词条 |
...
同义词检测不是形成知识图谱的唯一前提。只要 EDU 已经具有事件要素和角色，并且其参数在固定文档版本内可靠链接到 Entity，就可以形成 `EDU → Entity` 边；名称归一化的作用是让不同 EDU 指向同一实体，提高图的连通度。无法可靠归一化的事件要素继续保存在 EDU 中并参与文本、向量检索，不因未链接而丢弃，并作为画布显示节点。

与论文 EMem-G 的具体差异是：

- 论文以 Session、EDU、Argument 为节点；平台首期只把 EDU 和稳定实体作为核心图节点，未链接参数仅作显示节点，文档与文本单元通过来源引用导航。
...
未链接的 `edu_argument` 作为**显示节点**进入画布，查询时物化，不建第三类领域对象。点击后打开的是待链接事件要素，不是假 Entity。用户确认链接后，显示节点被 Entity 替换。冷启动和宁缺毋错时，若只画已链接 Entity，图会接近空白。

文档版本和文本单元作为过滤属性、来源着色和来源导航，不必变成图节点。时间首先作为 EDU 的结构化字段，不必建立时间节点网络。跨来源事件组是查询时聚合视图，不落成事件簇表。

### 10.4 边

核心边由 EDU 的事件要素和文档级实体链接产生。参数值已经链接为 Entity 时，按第 12 章的粗粒度固定角色生成对应边；`subject`、`object`、`participant`、`location`、`origin`、`destination`、`instrument`、`value` 和 `other` 均适用。未链接参数用点线接到显示节点。时间值继续保存在 EDU 字段中，不重复生成实体边。

首期画布只画 EDU → 实体（含未链接显示节点）的二部图：
...
首期画布不画 EDU–EDU 边。关系型引用和跨来源对照放在事件页相似事件区、EDU 检查器和查询时事件组。后续若固定跳或浏览因缺少该边而稳定漏召回，再引入 `edu_ref` 画布边。
...
这些信号用于检索和查询时聚合的事件组，不默认合并记录，也不建 `event_cluster` 表。事件组是可导航的一组 EDU，不是平台裁定「这是同一个历史事件」。

### 10.8 图查询范围

图查询只有一种实现：在给定的有权文档版本集合上，由 EDU 参数和文档级实体链接投影子图。三种产品 scope 只换集合，不复制链接、不另跑合并作业：

| Scope | 文档版本集合 | 默认用途 |
| --- | --- | --- |
| 文档图 | 当前这一份固定版本 | 核对抽取、改链、单篇内部关系 |
| 项目图 | 项目所选且用户有权的文档版本 | 有项目时的默认工作图 |
| 全局图 | 当前用户有权访问的全部文档版本 | 跨项目发现同一实体 |

全局图靠共享 Entity 自动跨文档连接。一个实体在当前 scope 内没有任何可见 EDU 时，该实体不能出现，避免通过存在性泄漏无权材料。

默认使用一到两跳固定扩展；画布默认一跳，二跳需明确打开：
...
复杂、多种子和多路径问题可以切换到查询级局部 PPR。两种方式都必须设置节点数、边数、跳数、时间和返回数量预算。前端画布另设节点硬顶（建议 500），超出按度数截断并明确提示，不能静默丢节点。不对项目或全局做无约束全图导出。
...
溯知的边来自模型抽取和实体链接，置信必须在界面上可见，不能只靠一条统计。使用已有字段，不新增 EDU 列：

| 维度 | 字段 | 图上用法 |
| --- | --- | --- |
| EDU 状态 | `status` | 默认只显示 `active` |
| 定位精度 | `locationPrecision` | 徽章；降级必须可见 |
| 外部知识 | `usedExternalKnowledge` | 警示徽章 |
| 复核 | `reviewResult` | 检查器内展示 |
| 链接来源 | 自动 / 人工改链 | 实线或带人工标记 |
| 未链接 | 无实体链接 | 点线接到显示节点 |
| 纠错建议 | `entity_link_suggestion` | 不进图，进统计和队列 |

上下文补全后才链接（参数 `value` 与 `sourceForm` 不同）用虚线。节点上只留一个主徽章，细节进悬停和检查器。每个区块说明「这是来源陈述，不是正确性主张」。
...
- 图 scope 对应的有权文档版本集合、时间、角色和节点类型约束；
...
「实体与事件」做成图谱、实体、事件三个并列视图，含列表、实体页、事件页和画布。不把「图谱」提为第六个一级入口。评审属于阅读和项目工作流，不单独建设脱离上下文的复杂后台。登录后仍以文档库为第一公民，不为图资产另建应用首页。
...
文档库可用一个顶区或视图收拢抽取进度、待复核和纠错建议，避免任务状态只散落在各文档详情里。这不是新的一级入口，也不取代文档列表。
...
项目页顶区展示纳入的文档版本数、EDU 覆盖、链接率、图密度和最近抽取/链接，并提供打开项目图的入口。这些数字与图谱页统计条使用同一口径，先按有权文档过滤再聚合。

### 16.4 阅读与 EDU 工作台

建议采用三栏布局：

- 左侧：由 `path` 聚合的章节目录树（卷 / 篇或纪 / 年）；
- 中间：当前目录节点下的固定版本原文各段；一次只渲染该前缀，避免整书进页；
- 右侧：当前段的 EDU。

选择一个 EDU 时，在原文中高亮其主要原文和补全上下文。具有文档编辑权限的用户可以在右侧编辑自足表述、角色、时间、状态和该固定文档版本的实体链接；其他有权用户只读查看，并可以对实体链接提交不直接改变当前结论的纠错建议。

右侧按句或文本单元聚合 EDU。卡片至少能看到自足表述、状态、主要谓词和角色；结构化字段默认折叠，选中后再展开。用户如果在卡片上看不到角色，就不会信任画布上的边。阅读工作台只查看和修正已经批量生成的 EDU，不在阅读时触发新抽取。

当前范围没有 EDU 时，空状态说明原文仍可阅读和检索，并提供“前往文档库抽取”链接。链接可以携带当前文档版本和范围作为尚未提交的表单参数，但必须回到文档库确认范围和预算后才创建任务。文档详情提供「在图中查看本版本」，打开文档图 scope。
...
- 规范显示名和已知别称；
- 平台共享实体池中的身份索引记录、当前项目是否关注，以及添加或移除关注的操作；界面说明该记录用于跨文档检索，不是人物传记或对争议身份的权威裁定；
...
任何关系点击后进入对应 EDU 和原文。实体页可内嵌一跳关系小图，并提供「在图谱中查看」。

每条关系和 EDU 默认带文档题名、版本、限定和复核来源；来自不同文档的矛盾表述并陈，不能把实体页表现成平台裁定后的客观传记。不为实体生成综述词条。

从项目进入实体页时，只展示该项目所选文档版本中的关系；从全局入口进入时，展示用户有权访问文档中的关系。两种视图使用同一份文档级实体链接，不产生项目口径分歧。

### 16.6 事件页

事件型 EDU 页面显示完整自足表述、参与者、时间、地点、谓词、来源、补全和复核来源。相似事件区列出可能来自其他文档的相关 EDU，但默认不自动合并；首期这是 EDU–EDU 关系的产品出口，不在画布上画 `edu_ref`。时间未知时按文档叙事序定位，不制造精确时间轴。提供「在图谱中查看」跳到以该 EDU 为中心的邻域。
...
具有文档编辑权限的用户可以按需人工复核、批量接受或拒绝。拒绝操作不得表述为“从当前项目移除”；确认前必须说明：拒绝表示该 EDU 错误或对所有使用范围均无有效信息，并会影响所有使用该固定文档版本的项目。人工结论覆盖模型结论；平台统计模型复核被人工推翻的比例，用于调整复核提示与自动采用策略。

质量抽样同时覆盖模型复核通过项和疑点项：通过桶做小比例随机抽样，疑点桶按类别分层抽样，并对禁止外部知识样本单独报告。抽样不阻塞自动采用；平台分别统计疑点命中率、通过桶漏错率和加权后的复核后残余错误率，不能只用疑点项推翻率评价复核质量。

### 16.9 图谱视图

「实体与事件」做成三个并列视图，不新增一级入口：

- 图谱：画布、统计条、页内搜索、检查器；
- 实体：列表与实体页；
- 事件：EDU 列表与事件页。

从文档详情「在图中查看本版本」进入文档图；从项目进入默认项目图；全局图在图谱页切换 scope。URL 保存 scope、选中节点、跳数、类型过滤和着色模式。

布局：

```text
顶栏：scope（文档/项目/全局）│ 着色（类型/来源文档）│ 筛选
统计条：实体数 │ EDU 数 │ 边数 │ 链接率 │ 自动/人工 │ 定位精度 │ 待处理纠错
左：力导向画布
右：未选中时为搜索 + 高连接实体 + 图例；选中后为检查器
```

顶栏约定：

- scope：有项目时默认项目图，从文档进入则为文档图，全局需明确切换；
- 着色：默认按类型（实体类型与 EDU 类型），可切换为按来源文档；语义社区着色不进首期；
- 筛选与页内搜索：类型、角色、来源文档、EDU 状态、是否已链接。页内搜索面向结构浏览，编译为当前 scope 的 SQL 条件，不替代检索页；全局命令面板放到阶段 D。

统计条口径与质量门 11、12、13 对齐，必须先按有权文档过滤再聚合。

画布可平移、缩放、点选，并允许拖动节点微调布局；布局不落库。不提供在图上改边或改实体的入口；修正回到阅读器和实体页。默认一跳，二跳需明确打开。前端节点硬顶建议 500，超出按度数截断并提示已隐藏数量，不能静默丢节点。可以把当前节点的一到两跳提出、其余降噪；这是浏览辅助，不改变查询语义。

节点标签：

- EDU 显示短标签（主要谓词加一到两个角色值），完整自足表述放检查器；
- Entity 显示规范名，副标签为类型；节点大小表示当前范围内提及次数，不是平台重要性；
- 未链接显示节点形状或描边弱于 Entity；点击打开的是待链接事件要素，不是假 Entity。

检查器是对应页面的缩略版，不要为图另做一套属性模式：

| 选中 | 检查器内容 |
| --- | --- |
| Entity | 规范名、别称、是否关注、链接条数、相关 EDU、按角色分组的边、来源分布、合并影响面 |
| EDU | 自足表述、类型、谓词、角色、时间、来源摘录、补全、状态、定位精度、相似事件组 |
| 未链接参数 | 原文写法、规范值、建议类型、所在 EDU、候选 Entity |

检查器固定提供：打开完整实体页或事件页；在画布显示邻域；打开固定版本原文并高亮文本单元。每条边必须能从检查器完成三跳回原文。

图面固定说明：这是当前范围内来源材料的关联视图，不是客观世界知识图谱；未链接节点不是新人物；不同文档的边可以同时存在，并不表示平台已裁决哪一条为真；全局范围只包含当前用户有权阅读的材料。

阅读面保持浅色工作台；图面可以使用深色画布，进入图谱视图再切换。
...
| 文档解析   | 内置 TXT/Markdown 导入器与结构方案；阶段 P 再接 MinerU Web API | B1 完成全书目录与段级 unit；PDF、Office、图片和 HTML 在写作之后接入；原始解析结果进入 S3 |
...
flowchart TB
    ...
    API --> JOB[任务执行器]
    JOB --> PARSER[文档解析器接口]
    PARSER --> TEXT[内置 TXT/Markdown + 结构方案]
    PARSER --> MINERU[MinerU Web API · 阶段 P]
    JOB --> LLM[Spring AI 模型网关]
    JOB --> DB
    JOB --> FS
```

平台应用和任务执行器可以来自同一代码库、共享同一领域模型。B1 只走内置导入器和结构方案；MinerU 在阶段 P 接入。首期不需要 Quartz、消息中间件、图数据库、工作流引擎或微服务网关。

### 17.4 逻辑模块

| 模块                            | 职责                                   |
| ----------------------------- | ------------------------------------ |
| 文档管理（Document Management）     | 导入、结构方案与解析预览、版本、文本单元、权限和原始资产   |
...
文档管理只依赖文档解析器（Document Parser）接口，不依赖 MinerU 的任务状态、下载地址或结果目录结构。B0/B1 使用内置 TXT/Markdown 导入器和结构方案；`MineruWebApiDocumentParser` 在阶段 P 接入。后续解析器使用相同契约：
...
TXT 和 Markdown 在用户点击“提取文档内容”后使用平台内置的同步文本导入器，通过同一标准化契约生成文档版本和文本单元，不发送到外部解析服务。全书 txt 先按结构方案做解析预览，确认后再写版本。阶段 P 才把 PDF、Office、图片和 HTML 交给外部解析器；在此之前不要为 PDF 预留空适配器。解析器标题映射到 B1 同一套 `path`。
...
阶段 P 认可外部解析器返回结果，不自行执行 OCR 正确率评估或页面图像逐字校对。平台只检查结果能被读取、文本单元顺序与页码映射形状合法、模型引用能在返回文本中定位。解析器、配置和原始结果固定进文档版本；用户重新解析或修正 OCR 时生成新版本。解析服务与模型服务适用同一数据边界，受限材料不能发送给未获许可的外部解析器。
...
阶段 P 优先使用 MinerU 提供的文件上传 URL：任务执行器从平台 S3 读取对象并流式上传到 MinerU，不把平台存储桶改成公开访问。MinerU 文档明确提示其 URL 解析可能无法访问 GitHub、AWS 等境外地址，因此不能默认把平台的 S3 预签名 GET URL 交给 MinerU 拉取。只有部署区域的连通性测试通过后，才能启用 URL 直取路径。

阶段 P 由平台后台轮询 MinerU，并使用逐步退避控制频率。暂不接收 MinerU 回调，避免增加公网回调鉴权、校验和验证、重复通知幂等处理；当外部轮询量成为可测瓶颈时再启用回调。
...
前端只轮询平台任务接口，不直接轮询 MinerU。任务详情页可在运行时约每两秒查询一次，后台列表降低到约五至十秒，页面不可见时进一步降频，进入成功、失败或取消状态后停止。轮询间隔是前端参数，不写入领域模型。阶段 P 接入 MinerU 后仍遵守这条规则。
...
| `entity_link_suggestion` | 对文档级实体链接的轻量纠错建议、目标修订、原因、建议 Entity 和处理状态 |
| `entity`             | 类型、规范显示名、修订号和工作空间边界列；不归属于单一项目 |
| `entity_name`        | 别称、原文称呼、检索名称和工作空间边界列             |
...
首期不需要通用 `node`、`edge` 表。EDU 的事件要素由 `edu_argument` 表达，文档级实体链接由 `edu_argument_entity_link` 表达；未链接显示节点和三种 scope 都是查询投影，不另存合并图。不额外物化实体间宽关系快捷边。
...
5. 图中的每条 EDU—Entity 边必须能回到文档级实体链接或未链接参数，并经 EDU 与来源引用三跳回到固定版本原文；三种 scope 只能过滤有权文档版本，不能复制或覆盖链接。
6. 回答和文章引用必须能回到有权限访问的文档版本。
7. 外部知识补全不能伪装成上下文补全。
8. 被拒绝或替代的 EDU 不进入默认检索，但历史成果仍能解析它；与当前项目无关不能成为拒绝理由。
9. 原文删除或权限变化不能留下看似有效但无法展开的公开引用。
10. Entity 属于工作空间共享实体池；项目关注和文档级实体链接都引用 Entity，不复制实体。
...
16. 实体链接纠错建议不进入检索或图投影；只有文档编辑者基于当前修订执行的实际改链才能改变链接结论。
17. 图子图与统计条必须先按有权文档版本过滤再聚合；当前 scope 内没有任何可见 EDU 的实体不得出现。
...
| 删除无人引用的文档 | 按保留策略删除文档记录及资产 | 删除对应全文、EDU、实体链接、纠错建议、向量和图投影 | 有成果引用时禁止硬删，改为不可用并提示 |
...
不暴露 `/evidences`、`/mentions`、`/claims`、`/interpretations` 或通用 `/graph/nodes` 管理接口。`/graph` 只提供邻域查询和当前 scope 统计，不提供写边、建合并图或导出全图。邻域查询接受 `scope`（document / project / global）、中心节点和跳数，返回当前有权范围内的 EDU、已链接实体、未链接显示节点和边；统计使用同一过滤后再聚合。
...
| 成果有效性检查   | EDU 被拒绝、替代、引用目标失效或访问权限变化 | 需要复核的成果列表      |
| 实体变更影响预览  | 文档级实体改链、Entity 合并或拆分 | 受影响的文档版本、EDU、链接和待重建图投影统计 |

只有成果有效性检查会把成果标记为“需要复核”。文档级实体改链和 Entity 合并或拆分只更新后续检索与图投影，不因实体索引变化把已有成果标记为失效。
...
实体链接纠错建议只使用待处理、已采纳、已驳回三种状态，保存提交者、时间、目标 EDU 参数和链接修订、原因、可选建议 Entity 及处理者。处理时目标修订已经变化的建议不能直接应用，必须由文档编辑者基于当前链接重新判断；采纳产生的实际改链继续使用上一段的审计记录。
...
首版按单一工作空间部署，不建设组织层级或工作空间切换界面。共享实体表预留工作空间边界列。登录用户只访问自己有权的项目、文档、成果和任务；权限条件在数据库查询与索引召回时直接执行，不能先召回无权内容再由应用层过滤。图子图与统计条同样先过滤再聚合。

首期文档权限只设三种角色：

| 角色 | 权限 |
| --- | --- |
| 查看者 | 阅读和检索；在另有项目编辑权限时可以把文档版本加入项目；可以提交实体链接纠错建议，但不能修改文档级语义记录 |
| 编辑者 | 包含查看者权限；可以发起内容提取、EDU 抽取和实体链接任务，并修改、拒绝或替代 EDU，解除或修改实体链接 |
| 所有者 | 包含编辑者权限；可以授予和回收文档访问权、转移所有权，并在引用与保留规则允许时删除文档 |

文档创建者默认成为所有者，文档至少保留一名所有者；平台管理员只用于权限恢复和平台级操作。项目角色不授予文档权限，也不能把查看者提升为文档编辑者。

平台共享实体池中的实体名称和类型可用于候选匹配，但实体页和图浏览只显示当前用户有权文档产生的 EDU、关系与来源。某实体在当前 scope 内全部关联 EDU 都无权时，该实体不得出现。

EDU 和实体链接属于固定文档版本，只有该文档的编辑者或所有者才能修改、拒绝、替代、解除或改链；查看者提交的纠错建议不改变文档级语义。共享 Entity 的合并和拆分属于平台级操作，只允许具有平台级实体管理权限的用户执行，并在操作前显示权限安全的受影响文档版本、EDU 和链接数量。

EDU、实体链接纠错建议、实体图边、向量和研究成果不能扩大原文权限。首期每条 EDU、实体链接及其纠错建议继承所属固定文档版本的权限，不跨文档版本形成 EDU。
...
受限材料可以选择本地模型、本地解析器、脱敏处理或禁止模型分析。阶段 P 接入 MinerU 时仍不要求第二解析器，但不能把 MinerU 可用等同于材料获准外传。
...
13. 图浏览中任意可见边能三跳回到有权原文；无权 EDU 不进子图和统计条；当前 scope 内无可见 EDU 的实体不出现。

| 适用阶段 | 必须满足的质量门 |
| --- | --- |
| 阶段 A | 1、4、6、7，并形成继续或降级结论 |
| 阶段 B | 2、3、8、9，形成可用的文档到 EDU 检索闭环 |
| 阶段 C | 11、12、13，实体链接和图不得破坏基础召回，图浏览不得泄漏无权内容 |
| D-min | 5，并满足第 22.4 节的单篇与跨文档问答指标 |
| D-scale | 10，完成大规模和经济性验证后才宣称 L1 路线成立 |

具体百分比阈值在建立至少两个领域的稳定金标准后确定，本文不预设没有实验依据的数字。A2 负责产出阈值制定方法、基线分布、最低样本要求和不确定性说明；各阶段在开始前依据可用金标准冻结本阶段的数值门槛。D-min 的正式门槛在阶段 D 开始前，依据阶段 C 形成的单篇、跨文档问答题集和引用金标准确定，不要求 A2 提前制造证据不足的百分比。

扫描件不把 OCR 字符准确率作为平台质量门；平台认可解析器返回并固定的文本，只检查结构可用性和 EDU 引用能否回到该解析文本。文本 PDF 与 OCR PDF 的三级定位分报在阶段 P 补齐。页面图像与 OCR 的逐字质量评估属于延后能力。

---

## 23. 分阶段实施路线

阶段不是功能清单，而是逐级验证核心假设。每一阶段开始前固定问题集、语料、权限范围、模型、预算、指标阈值和停止条件，结束时根据质量和成本选择继续或降级。实际开工前另行形成包含参与人员、样本规模、预算和日历安排的执行计划；本文不预设缺少团队与实验依据的工期数字。

仓库当前从 B1 开工。顺序为 B1（全书结构与阅读器）→ B2 → B3 → C → D-min → D-scale → E → P（PDF、MinerU、OCR）→ F。不要把 MinerU 提前到 B1。

| 路线 | 实验结论 | 可交付形态 |
| --- | --- | --- |
| L1 | 批量 EDU、实体链接和图均产生稳定净收益 | 按 A 至 E 推进 TXT 上的完整平台，再进入阶段 P 接入 PDF |
...
- 后续阶段指标阈值的制定方法、基线分布、最低样本要求和不确定性报告，不提前写死证据不足的百分比。
...
- 一书一份结构方案、解析预览、目录树与段级文本单元；
- 三栏阅读器：目录树、当前节点原文、EDU；
- 单文档和多选文档的显式处理入口、范围与汇总预算确认；path 前缀抽取；全书默认禁止一键全文抽取 EDU；
- “抽取 EDU”按钮、批量生成、三级定位和状态；
- 确定性校验、默认模型复核、显式低风险规则、双桶质量抽样和高影响候选队列；
- 重复候选簇的保留、拒绝和替代操作；
- 中文规范检索文本、底本位置映射、规范化配置版本、原文全文与原文/EDU 向量检索；
- RRF 候选融合及带项目和文档权限的检索；
- EDU 编辑、拒绝、替代、乐观并发控制、拒绝的全局影响提示和模型结论被人工覆盖的记录；
- 阅读器无 EDU 空状态及返回文档库的未提交范围入口；
- 阅读器 EDU 卡片展示自足表述、状态、主要谓词和角色，选中后展开结构化字段。

此阶段不建设实体图、文章生成或查询时按需 EDU。向量与 RRF 在 B3 完成。PDF、MinerU 与 OCR 不在本阶段，放到阶段 P。

退出条件：从上传到可检索 EDU 的任务可恢复、可重跑且不越权；原文与 EDU 对照在固定问题集上完成；模型复核不依赖人工逐条审核即可形成可用检索池；TXT/Markdown 报告三级定位分布，并给出代表性文档处理成本、吞吐、索引体积和容量基线。代表性长文档的目录导航、原文高亮和引用锚点在可接受延迟内工作，规范文本映射不能造成展示位置漂移。文本 PDF 与 OCR PDF 的定位分报在阶段 P 补齐，不挡本阶段在 TXT 上退出。

### 阶段 C：实体关联图

交付：

- 工作空间共享实体池（表带 `tenant_id` 占位）、可选项目关注实体、别称和固定文档版本内 EDU 参数的实体链接；
- 冷启动种子、未链接保留并作为画布显示节点、文档级改链、查看者纠错建议、共享 Entity 合并拆分权限和文档影响预览；
- 实体页、事件页，以及「实体与事件」下的图谱视图（统计条、页内搜索、检查器、力导向画布）；
- 文档 / 项目 / 全局三种 scope，同一套邻域查询，不另建合并图；
- 查询时事件组（不建事件簇表）；
- 统一图检索策略接口；
- 默认的一到两跳固定扩展，与画布邻域共用后端；
- 查询级局部 PPR 实验实现；
- 以文档叙事序为基础、规范时间为增强的人物时间线；
- 来源陈述、语义限定、矛盾并陈和相关事件浏览；
- 与阶段 D 共用的单篇、跨文档问答题集及引用金标准。

图关系全部从 EDU 参数的固定角色和文档级实体链接产生。三种 scope 只过滤有权文档版本。不引入宽关系快捷边、画布 EDU–EDU 边、通用图数据库或实体综述生成。阶段 C 不为 EDU 增字段。力导向画布后置于列表、统计和检查器，不挡实体链接质量主线。

本阶段对三组检索进行消融比较：向量加全文、前者加固定跳、前者加局部 PPR。实体链接先按分领域金标准通过正确率、错链率和未链接率门禁，并分别验证共享实体池正常和冷启动两种情况；另行验证项目关注实体为空或已设置时都不会改变自动链接结论，也不得压低集合外实体的基础召回。查看者可以提交纠错建议但不能直接改变链接，编辑者采纳后才重建图投影；目标修订已变化的建议不能静默覆盖当前结论。图浏览另行验证用户是否会误把来源陈述读成平台事实，以及无权内容是否出现在子图或统计条。局部 PPR 只有在多跳问题的 Recall@K、回答正确率或噪声率上获得稳定收益，并满足延迟预算后，才允许进入自动策略选择。
...
写作闭环不依赖 PDF。PDF 解析、MinerU 和 OCR 排在写作之后。

### 阶段 P：PDF 解析、MinerU 与 OCR

交付：

- 可替换的文档解析器；
- MinerU 网页版解析（默认）；
- 扫描页 OCR 为 Markdown 或纯文本；
- 页面图像、坐标和 bbox 的解析产物；
- 解析记录与 `document_file.extracted_content_id`；
- PDF 定位分报：解析/OCR 后 bbox 与原文偏移的对应情况。

约束：不把 MinerU 和 OCR 提前到 B1–B3。TXT/Markdown 闭环已经在 B3 退出。扫描件、bbox、PDF 阅读器和 OCR 质量评估都在本阶段接入。

退出条件：扫描 PDF 可走完解析、切分、EDU 抽取、检索和阅读；PDF 定位分报可复现。敏感材料需要本地解析器时，放到阶段 F。
...
随着需求增加，容易重新建设多层节点和边。控制方式：图边必须来自 EDU 参数；未链接显示节点不得升级为领域对象；新增节点类型前证明其具有独立生命周期和查询价值；画布 EDU–EDU 边与语义社区着色走第 25 章。

### 24.5 实体错误合并污染大量 EDU

控制方式：项目关注实体不参与自动链接，共享实体池信息只用于产生和排序候选；固定文档版本内容不足以可靠区分候选时保持未链接。错链在固定文档版本内解除或改链并保留修订历史；共享 Entity 合并需要平台级权限和文档影响预览，支持拆分，不按同名自动合并，历史成果不自动改写。
...
向量检索、图扩展和实体浏览可能遗漏项目或文档权限。控制方式：数据库查询和派生索引直接应用资源权限；图子图与统计条先过滤再聚合；当前 scope 内无可见 EDU 的实体不得出现。集成测试覆盖全文、向量、实体页、图邻域、图统计、任务和预签名资源。
...
| 延后能力        | 引入条件                                 |
| ----------- | ------------------------------------ |
| 跨来源事件簇      | 用户需要稳定比较同一事件的多来源版本，并且聚类准确率可接受；当前用查询时事件组 |
| 画布上的 EDU–EDU 边 | 固定跳或浏览因缺少 `edu_ref` 而稳定漏召回 |
| 语义社区着色      | 类型着色、来源文档着色和事件组不足以支持主题发现，且聚类不造成裁决误读 |
| 物化图投影        | 实时邻域查询无法达到延迟目标                 |
| 由团块生成实体综述  | 用户需要叙事正文且能避免读成平台传记；当前用实体页、事件页和检查器 |
...
```text
导入一份文档
→ 全书 txt 选择结构方案并预览目录（短文本可跳过）
→ 用户显式提取文档内容并固定原始资产和解析版本
→ 形成稳定文本单元
→ 用户确认范围和预算后批量生成事件式语义单元
→ 在 EDU 内完成实体、指代和时间补全
→ 确定性校验并由模型默认复核，低风险通过项自动采用
→ 将固定文档版本中的 EDU 参数链接到工作空间共享实体池，无法可靠认定时保持未链接并作为图上显示节点
→ 从文档级 EDU 和文档级实体链接投影关联图，再按文档、项目或全局三种有权范围过滤
→ 联合原文、EDU、实体和图进行检索
→ 使用模型筛选少量高相关语义记忆
→ 生成带引用的回答或研究成果
→ 从回答回到 EDU，再回到固定版本原文
→ 查看者可以提交不直接改变实体链接的纠错建议，文档编辑者可以按需人工复核并覆盖模型结论
→ EDU 失效时识别需要复核的成果，实体变更时预览文档级索引和图投影影响
```
...
这套设计的核心取舍是：**保留原文，做好 EDU，让图成为可逛的第二界面，最终仍回到原文。**
