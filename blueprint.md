...
> 修订日期：2026-09-14
>
> 文档性质：产品、领域、模型与实施规划。B0 已按 1.13 落地；1.14 锁定 B0 范围，并写入图谱产品形态。
>
> 核心路线：文档 → 事件式语义单元（Event-like EDU）→ 实体关联图（Entity Association Graph）→ 检索与生成 → 原文引用
>
> 图谱产品讨论底稿：[graph-grok.md](./graph-grok.md)。与本文冲突时以本文为准。
...
本规划定义产品、领域与实施边界。仓库已按 1.13 落地 B0（文档、固定版本、TXT/Markdown 抽取、EDU 持久化、基础检索与阅读器）。1.14 不重开 B0，只把后续阶段需要的图谱浏览、图查询范围和置信呈现写进本文。
...
此阶段不建设实体图、文章生成或查询时按需 EDU。向量与 RRF 在 B3 完成；角色展示不阻塞 B1 的 PDF 入口。
