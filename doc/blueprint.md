...
| D08 | 实体图从文档级 EDU 和项目实体链接投影           | 图可以重建，不形成第二套权威语义                           |
...
| D13 | 纠正通过替代 EDU 或修改项目实体链接完成           | 不为评审另建复杂判断图                                |
...
| D18 | 首期 EDU 全部是文档级语义记录                  | 项目问题、笔记和解释只进入检索、回答与研究成果，不参与 EDU 生成          |
| D19 | Entity 共享，实体链接结论按项目保存              | 同一文档级 EDU 可在不同项目中形成不同链接；项目改链不影响其他项目          |
...
文档解析完成后，用户可以在文档库对整个版本或指定范围发起 EDU 抽取。系统仍按原有逻辑把文档拆成多个 Text Unit，以一个句子、段落或一组连续文本为分析目标，并组合标题、前后文、传主身份和文档内纪年范围形成上下文。首期 EDU 全部只使用文档固有上下文生成。项目关注实体只在模型调用后用于项目内实体链接，不进入 EDU 生成上下文，也不改变可复用的文档级 EDU 文本。
...
EDU 生成后的默认流程是确定性校验加大模型复核。模型复核检查忠实性、遗漏、指代、时间、角色和禁止推断；只有满足第 12.6 节显式低风险规则且复核通过的 EDU 才自动进入 `Active`，其他项保持 `Proposed` 并按疑点或影响进入队列。人工复核按需进行，不阻塞批量处理；人工结论优先于模型结论，覆盖时必须保存修改前版本、操作者和理由。模型复核不得把模型自报置信度当作证据，也不得直接覆盖已有 EDU。
...
| 项目级 | 选定文档版本、项目关注实体、实体链接、项目配置和研究成果 |
...
首期 EDU 全部只使用文档内部上下文生成，并在权限允许范围内复用。项目关注实体、项目笔记或跨文档背景只影响项目内实体链接、检索、回答与研究成果；确实依赖项目解释才能成立的内容属于研究成果，不生成项目级 EDU。项目级 EDU 只有在真实任务证明研究成果不足以承载、且作用域、去重和检索优先级均已验证后才引入。

实体身份在平台内共享，实体链接结论按项目保存：文档级 `edu_argument` 保留角色、原文写法、规范值和建议类型，项目级链接记录把具体参数连接到共享 Entity。同一文档版本被多个项目使用时，各项目可以形成不同链接；项目内改链不修改文档级 EDU，也不影响其他项目。
...
- 人工复核不是进入 `Active` 的必要条件，但人工结论优先，并通过替代 EDU 或修改当前项目实体链接生效；
...
这样可以直接表达“太宗在当前 EDU 中被补全为李世民”，不需要单独的原文指称对象。正式实体链接按项目另行保存，不写回可复用的文档级 EDU 参数。
...
链接结果保存为当前项目、EDU 参数与共享 Entity 之间的关联。项目关注实体影响候选顺序，但不修改文档级 EDU；同一 EDU 在不同项目中可以保留不同链接结论。
...
错误实体链接优先在当前项目内解除或改链；只有确认两个共享 Entity 表达同一身份时才执行实体合并，实体误合并时可以拆分。项目内改链只更新当前项目链接；共享 Entity 的合并或拆分会更新所有受影响项目的实体链接，并保留操作日志。

共享 Entity 的合并与拆分需要平台级实体管理权限。操作前必须跨项目显示受影响的项目、EDU、实体链接和研究成果数量；预览遵守资源权限，不能向无权用户泄露项目或来源详情。已发布研究成果继续引用当时版本，并提示需要复核。首期不建立复杂的可撤销身份推理网络或变更通知中心。
...
同义词检测不是形成知识图谱的唯一前提。只要 EDU 已经具有事件要素和角色，并且当前项目把参数可靠链接到 Entity，就可以形成 `EDU → Entity` 边；名称归一化的作用是让不同 EDU 指向同一实体，提高图的连通度。无法可靠归一化的事件要素继续保存在 EDU 中并参与文本、向量检索，不因未链接而丢弃。
...
核心边由 EDU 的事件要素和当前项目的实体链接产生。参数值在当前项目已经链接为 Entity 时，按第 12 章的粗粒度固定角色生成对应边；`subject`、`object`、`participant`、`location`、`origin`、`destination`、`instrument`、`value` 和 `other` 均适用。时间值继续保存在 EDU 字段中，不重复生成实体边。
...
上下文按总词元预算裁剪，推荐顺序是：完整保留分析目标，优先保留直接相邻文本和所属篇章，再加入文档概览。分析目标过长时先切成多个重叠目标，不通过压缩目标原文给背景让路。项目关注实体、项目问题、笔记和跨文档背景不进入首期 EDU 生成上下文，只在生成后参与项目内实体链接、检索、回答和研究成果。
...
查询中的名称、别称或描述先匹配当前项目关注实体，再在平台共享实体池中补充候选。关注实体只提高消歧与排序优先级，集合外实体仍可被链接和召回。例如“那个皇太子”“太宗”“建成”等可以通过实体名称和 EDU 原文写法找到相关记录。实体入口和图检索使用当前项目保存的实体链接，不把其他项目的链接结论当作本项目事实。
...
选择一个 EDU 时，在原文中高亮其主要原文和补全上下文。右侧直接编辑自足表述、角色、当前项目实体链接、时间和状态。
...
| `project_edu_argument_link` | 项目、EDU 参数与共享 Entity 的链接结论及修订号 |
...
首期不需要通用 `node`、`edge` 表。EDU 的事件要素由 `edu_argument` 表达，项目内实体链接由 `project_edu_argument_link` 表达；不额外物化实体间宽关系快捷边。
...
5. 图中的每条 EDU—Entity 边必须能回到当前项目的实体链接及产生它的 EDU 参数。
6. 回答和文章引用必须能回到有权限访问的文档版本。
7. 外部知识补全不能伪装成上下文补全。
8. 被替代 EDU 不进入默认检索，但历史成果仍能解析它。
9. 原文删除或权限变化不能留下看似有效但无法展开的公开引用。
10. Entity 属于平台共享实体池；项目关注和项目实体链接都引用 Entity，不复制实体。
...
14. EDU 替代、项目实体改链和共享 Entity 合并必须校验修订号；过期写入不能静默覆盖新结果。
...
| Entity 合并或拆分 | 需要平台级实体管理权限，保留操作日志和旧链接版本 | 重建全部受影响项目的链接和图投影 | 标记需要复核 |
...
- 所有原文读取固定到文档版本；
- EDU 替代、项目实体改链和 Entity 编辑使用修订号或 ETag；版本冲突返回 `409 Conflict`，不静默覆盖；
...
| 实体链接      | 项目、EDU 参数、项目关注实体和共享实体池 | 项目实体链接或候选      |
...
普通项目成员可以在有权项目内解除或修改实体链接。共享 Entity 的合并和拆分属于跨项目操作，只允许具有平台级实体管理权限的用户执行，并在操作前完成权限安全的影响预览。
...
- 平台共享实体池、可选项目关注实体、别称和 EDU 参数的项目级实体链接；
- 冷启动种子、未链接保留、项目内改链、共享 Entity 合并拆分权限和跨项目影响预览；
- 实体页和事件页；
- 统一图检索策略接口；
- 默认的一到两跳固定扩展；
- 查询级局部 PPR 实验实现；
...
图关系全部从 EDU 参数的固定角色和当前项目实体链接产生，不引入宽关系快捷边或通用图数据库。
...
控制方式：项目关注实体只提高候选优先级、平台共享实体池内谨慎匹配、无法确定时保持未链接。错链优先在项目内解除或改链；共享 Entity 合并需要平台级权限和跨项目影响预览，支持拆分，不按同名自动合并，历史成果固定当时链接版本。
...
→ 在项目内将 EDU 参数链接到平台共享实体池，并由可选的项目关注实体提供软优先候选
→ 从文档级 EDU 和项目实体链接投影当前项目的实体关联图
→ 联合原文、EDU、实体和图进行检索
→ 使用模型筛选少量高相关语义记忆
→ 生成带引用的回答或研究成果
→ 从回答回到 EDU，再回到固定版本原文
