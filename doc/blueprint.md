...
| D16 | EDU 默认由模型复核，人工复核不作为流程卡点          | 模型复核通过后可进入默认检索；人工按需复核并可覆盖模型结论               |
| D17 | 项目关注实体是可选的软优先级                    | 可以为空；只影响实体消歧、候选排序和查询扩展，不限制 EDU 抽取或新实体发现     |
...
- **提取文档内容**：调用选定解析器生成新的不可变文档版本、文本单元和页图映射。如果已经是txt、md等文本格式，则不支持提取；
...
项目关注实体完全可选，可以为空。它只为实体消歧、链接候选排序、查询扩展和结果排序提供软优先级，不是实体白名单，也不限制文档内容提取、EDU 抽取或新实体发现。需要把问题严格限制在某些实体时，由用户在单次检索中显式设置过滤条件，不改变项目的长期配置。

在项目内上传文档，等价于先建立文档；内容提取完成后，再把用户选定的固定版本加入项目。

### 4.3 阅读与语义补全

文档解析完成后，用户可以在文档库对整个版本或指定范围发起 EDU 抽取。系统仍按原有逻辑把文档拆成多个 Text Unit，以一个句子、段落或一组连续文本为分析目标，并组合标题、前后文、传主身份和文档内纪年范围形成上下文。项目关注实体默认只在模型调用后用于实体链接，不进入文档级 EDU 的生成上下文，也不改变可复用的文档级 EDU 文本。
...
EDU 生成后的默认流程是确定性校验加大模型复核。模型复核检查忠实性、遗漏、指代、时间、角色和禁止推断，复核通过的低风险 EDU 可以自动进入 `Active`；有疑点的 EDU 保持 `Proposed` 并进入高影响队列。人工复核按需进行，不阻塞批量处理；人工结论优先于模型结论，覆盖时必须保存修改前版本、操作者和理由。模型复核不得把模型自报置信度当作证据，也不得直接覆盖已有 EDU。
...
| 文档级 | 文档版本、文本单元、仅依赖该文档上下文的 EDU |
| 项目级 | 选定文档版本、项目关注实体、实体链接、项目配置和确有必要的项目专用 EDU |
| 成果级 | 回答、备忘、提纲、文章及其引用映射 |
| 运行级 | 输入范围、提示版本、模型参数、费用、错误和产物摘要 |


首版按单一工作空间部署，不引入组织层级或额外的数据边界对象。Entity 不属于任何项目，而属于平台共享实体池。项目关注实体只是项目对一批 Entity 的可选标记，不建立另一份 Entity 副本，也不成为独立领域对象。添加或移除关注只改变后续候选排序和查询扩展，不修改 Entity、既有 EDU 或实体链接。

只使用文档内部上下文生成的 EDU 可以在权限允许范围内复用。项目关注实体、项目笔记或跨文档背景默认只影响实体链接与检索；确实依赖项目解释才能成立的 EDU 必须明确标为项目级，不能伪装成可复用的文档级 EDU。
...
文本单元不是固定词元分块。用于模型理解的上下文窗口可以跨越多个文本单元，并根据标题、人物、纪年、用户选定范围和上下文策略动态组装。项目问题只有在用户显式生成项目级 EDU 时才进入上下文。
...
- 确定性校验通过后，由复核模型检查语义质量；低风险且复核通过的 EDU 自动进入 `Active`；
- 复核模型发现疑点时保持 `Proposed`，供后续模型重审或人工按需处理；
- 人工复核不是进入 `Active` 的必要条件，但人工结论优先，并通过替代 EDU 或修改实体链接生效；
...
首期把解析器返回的标准化文本视为该文档版本的可引用文本，不自行判断 OCR 字符是否忠于页面图像，也不建设 OCR 质量评分。平台仍须检查模型返回的摘录确实存在于解析器返回的对应文本单元；无法精确命中时只允许降级到解析器给出的文本单元或页面位置，不能用模糊相似度把模型改写自动认定为原文。重新 OCR 或人工修复文本时创建新文档版本。
...
1. 当前项目关注实体中的规范名称和别名精确匹配，并提高候选优先级；
2. 平台共享实体池中的规范名称和别名匹配；
3. 类型、时间和共同事件约束；
4. 向量或模型候选判断；
5. 无法可靠确定时保留未链接的事件要素值或待处理实体候选。

项目关注实体为空时直接跳过第一步，不降低 EDU 抽取覆盖率；即使共享实体池也为空，模型输出的原文写法、规范值和类型仍保存在 EDU 参数中，无法可靠链接的内容保留为未链接值或实体候选。实体池完善后可以重新执行实体链接和图投影，不需要重新抽取 EDU。

系统不因字符串相同自动合并，也不要求模型直接创建正式实体。链接默认宁缺毋错；未链接值继续参与原文和 EDU 检索。冷启动时以当前文档传主、高频未链接候选和用户已添加的项目关注实体为种子，经模型复核或人工确认后进入平台共享实体池。新识别实体不自动成为项目关注实体，由界面提供按需添加操作。
...
核心边由 EDU 的事件要素产生。参数值已经链接为 Entity 时，按第 12 章的粗粒度固定角色生成对应边；`subject`、`object`、`participant`、`location`、`origin`、`destination`、`instrument`、`value` 和 `other` 均适用。时间值继续保存在 EDU 字段中，不重复生成实体边。
...
图查询只在当前项目和用户有权访问的文档版本内运行。默认使用一到两跳固定扩展：
...
阶段 A 必须比较单次结构化输出、先生成 EDU 文本再抽取要素，以及只生成 EDU 文本三种方式。默认模型复核是对候选结果的独立检查，不重新执行整套抽取；生成与复核的总费用都计入每条可用 EDU 成本。

复核至少使用与生成不同的提示和输出契约，不读取模型的隐式推理。阶段 A 比较同模型复核与不同模型复核的人工推翻率和成本，再决定默认组合，避免把同一次错误简单自我确认。
...
| 项目上下文（Project Context） | 显式允许使用的项目笔记、术语或背景       | 仅生成项目级 EDU        | 默认不加入      |
...
上下文按总词元预算裁剪，推荐顺序是：完整保留分析目标，优先保留直接相邻文本和所属篇章，再加入文档概览。分析目标过长时先切成多个重叠目标，不通过压缩目标原文给背景让路。项目关注实体不进入文档级 EDU 的生成上下文，只在生成后参与实体链接；用户显式要求项目级抽取时，项目上下文才进入输入并固定到处理运行。

初始预算可以按比例控制：分析目标约占 35%～50%，相邻上下文约占 20%～30%，所属篇章约占 10%～15%，文档概览约占 5%～10%。项目级抽取的项目上下文只使用剩余预算。比例是可测参数，不写死在领域模型中。
...
忠实性、指代和角色正确性不能仅靠 JSON Schema 判断。确定性校验通过后默认执行一次模型复核；复核只返回通过、疑点类别和可核查说明，不直接改写 EDU，也不依赖模型自报置信度。低风险且复核通过的 EDU 可以自动采用，高风险或有疑点的 EDU 保持候选。人工可以按需复核并覆盖模型结论，但不是批量流程的必要卡点。
...
查询中的名称、别称或描述先匹配当前项目关注实体，再在平台共享实体池中补充候选。关注实体只提高消歧与排序优先级，集合外实体仍可被链接和召回。例如“那个皇太子”“太宗”“建成”等可以通过实体名称和 EDU 原文写法找到相关记录。
...
- 根据项目文档中已链接实体和高频未链接候选生成待关注建议，用户确认后才加入；
- 明确标注关注只影响链接与检索优先级，不会排除其他实体；
- 移除关注不删除共享实体，也不解除既有 EDU 的实体链接。

待关注建议直接从项目已有链接和未链接候选计算，不建立独立建议表或审批流程。
...
- 规范名称和已知别称；
- 平台共享实体池中的规范身份、当前项目是否关注，以及添加或移除关注的操作；
...
批量审核不是所有 EDU 的人工必经步骤。系统先完成确定性校验和模型复核，自动采用低风险通过项；队列只优先展示模型复核发现的疑点和以下高影响问题：
...
| 中文检索   | PostgreSQL 中文分词扩展或经基准验证的 n-gram 方案                  | 对规范检索文本建立索引；具体扩展在阶段 B 用召回率、部署成本和维护状态选定                         |
...
中文检索不能直接依赖 PostgreSQL 默认分词。每个文本单元保留不可变展示文本，并派生只用于召回的规范检索文本：先执行 Unicode NFKC，再按受版本控制的简繁、异体和人名保护词典归一。引用位置始终以不可变展示文本为准；规范化改变长度时保存位置映射，检索命中先映射回展示文本再生成引用。中文分词扩展或 n-gram 的具体选择由阶段 B 基准确定，不为尚未验证的实现增加第二套搜索基础设施。
...
首期不需要通用 `node`、`edge` 表。EDU 与实体的关系已经由 `edu_argument` 表达；不额外物化实体间宽关系快捷边。
...
5. 图中的每条 EDU—Entity 边必须能回到产生它的 EDU 参数。
6. 回答和文章引用必须能回到有权限访问的文档版本。
7. 外部知识补全不能伪装成上下文补全。
8. 被替代 EDU 不进入默认检索，但历史成果仍能解析它。
9. 原文删除或权限变化不能留下看似有效但无法展开的公开引用。
10. Entity 属于平台共享实体池；项目通过项目关注实体引用 Entity，不复制实体。
...
| 操作 | 权威记录 | 派生数据 | 已有成果 |
| --- | --- | --- | --- |
| 项目移除文档版本 | 删除项目绑定，不删除文档版本和文档级 EDU | 删除该项目内链接与缓存，可重建 | 保留历史版本并标记访问或范围变化 |
| 删除无人引用的文档 | 按保留策略删除文档记录及资产 | 删除对应全文、向量和图投影 | 有成果引用时禁止硬删，改为不可用并提示 |
| EDU 被拒绝或替代 | 保留历史记录和关系 | 从默认索引和图投影移除或重建 | 标记需要复核，不自动改写正文 |
| Entity 合并或拆分 | 保留操作日志和旧链接版本 | 重建受影响项目的链接和图投影 | 标记需要复核 |
...
| 实体链接      | EDU 参数、项目关注实体和共享实体池 | 正式实体链接或候选      |
...
人工复核覆盖模型复核时，保留原模型结论、人工结论和生效后的 EDU 版本，用于计算模型复核被推翻率。该记录是 EDU 与处理运行的审计字段，不扩展成通用评审系统。
...
平台共享实体池中的实体名称和类型可用于候选匹配，但实体页只显示当前用户有权文档和项目产生的 EDU、关系与来源。

EDU、实体图边、向量和研究成果不能扩大原文权限。一个 EDU 引用了多个文档版本时，其可见范围不超过这些版本权限的交集。
...
8. 模型复核可以在不依赖人工逐条审核的情况下形成可用检索池，并持续统计人工推翻率。
...
12. 设置项目关注实体后，集合外实体仍能被发现、链接和召回；只有单次查询的显式过滤可以排除它们。
...
阶段不是功能清单，而是逐级验证核心假设。每一阶段开始前固定问题集、语料、权限范围、模型与预算，结束时根据质量和成本选择继续或降级：
...
- 文档导入、“提取文档内容”按钮和不可变版本；
- 文本单元与原文阅读；
- “抽取 EDU”按钮、范围与预算确认、批量生成、定位和状态；
- 确定性校验、默认模型复核、低风险自动采用和高影响候选队列；
- 中文规范检索文本、底本位置映射、原文全文与原文/EDU 向量检索；
- RRF 候选融合及带项目和文档权限的检索；
- EDU 编辑、拒绝、替代和模型结论被人工覆盖的记录。

此阶段不建设实体图、文章生成或查询时按需 EDU。

退出条件：从上传到可检索 EDU 的任务可恢复、可重跑且不越权；原文与 EDU 对照在固定问题集上完成；模型复核不依赖人工逐条审核即可形成可用检索池；给出代表性文档处理成本、吞吐、索引体积和容量基线。

### 阶段 C：实体关联图

交付：

- 平台共享实体池、可选项目关注实体、别称和 EDU 参数链接；
- 冷启动种子、未链接保留、合并拆分和影响提示；
- 实体页和事件页；
- 统一图检索策略接口；
- 默认的一到两跳固定扩展；
- 查询级局部 PPR 实验实现；
...
图关系全部从 EDU 参数的固定角色产生，不引入宽关系快捷边或通用图数据库。

本阶段对三组检索进行消融比较：向量加全文、前者加固定跳、前者加局部 PPR。实体链接先按分领域金标准通过正确率、错链率和未链接率门禁，并分别验证项目关注实体为空、已设置以及共享实体池冷启动三种情况；关注实体不得压低集合外实体的基础召回。图浏览另行验证用户是否会误把来源陈述读成平台事实。局部 PPR 只有在多跳问题的 Recall@K、回答正确率或噪声率上获得稳定收益，并满足延迟预算后，才允许进入自动策略选择。
...
退出条件：在单篇、跨文档及千级并逐步扩展到万级文档库的同题对照中，报告 EDU 路线相对原文分块 RAG 的回答正确性、引用支持率、宽范围召回、延迟、权限过滤、上下文词元和总成本，并给出不同重复查询量下的盈亏平衡点。若大规模候选膨胀使 EDU 优势消失，选择 L2、L3 或 L4，而不是只依据小样本继续扩建。
...
控制方式：项目关注实体只提高候选优先级、平台共享实体池内谨慎匹配、无法确定时保持未链接、合并前影响预览、支持拆分、不按同名自动合并、历史成果固定当时链接版本。

### 24.6 不同模型结果差异大

控制方式：固定上下文和契约、分项评测、保存生成与复核运行、默认执行不改写原结果的模型复核、统计人工推翻率，不依赖模型自报置信度。
...
默认模型复核可能漏掉系统性错误，或使批处理费用和延迟翻倍。控制方式：复核只做独立检查、不重做抽取；按疑点分桶抽样人工复核，统计推翻率和队列积压；某类低风险错误率稳定后才允许跳过或使用更小模型，某类高风险推翻率过高时停止自动采用。
...
简繁、异体和 Unicode 规范化可能错误改写人名，或因长度变化使引用偏移漂移。控制方式：展示底本不可变，检索文本派生生成，词典版本化并保护领域名称，保存规范文本到展示文本的位置映射，所有引用最终回到底本。

### 24.14 大规模文档库候选膨胀

在成千上万份文档上，宽范围问题可能产生过多候选，使召回、模型筛选费用和响应时间恶化。控制方式：分层召回、RRF、候选和上下文预算、按问题类型分桶评测，并在千级到万级文档规模上与原文分块 RAG 比较；没有稳定净收益时收窄 EDU 使用范围。
...
→ 将 EDU 参数链接到平台共享实体池，并由可选的项目关注实体提供软优先候选
→ 从 EDU 投影实体关联图
→ 联合原文、EDU、实体和图进行检索
→ 使用模型筛选少量高相关语义记忆
→ 生成带引用的回答或研究成果
